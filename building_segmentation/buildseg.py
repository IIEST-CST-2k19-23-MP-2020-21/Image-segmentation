...
# Please insert path to the input base directory
map_base_dir = ''

# Please insert path to input image directory
map_img_dir = ''

# Change the name of JSON file according to the file you are using
json_path = os.path.join(map_base_dir, 'annotation.json')
with open(json_path, 'r') as f:
    annot_data = json.load(f)

# Let us pick up few random images on which we will test our model
# You may change the number if you wish
...
annot_df.sample(3)  # For better understanding, may be skipped

full_df = pd.merge(annot_df, image_df, how='left',
                   left_on='image_id', right_on='id').dropna()
print(image_df.shape[0], '+', annot_df.shape[0], '=>', full_df.shape[0])
full_df.sample(3)   # For better understanding, may be skipped

# We are using bbox from COCO standard
...
for (c_ax, d_ax, f_ax), (c_id, c_df) in zip(m_axs,
    ...
    rgba_img = np.concatenate([img_data,
                               np.clip(np.expand_dims(127*out_img +
                                                      127, -1), 0, 255).astype(np.uint8)
                               ], -1)
...
# Dataset Splitting
# Labeled Data is used for testing
...
# Function to generate batch image
# Numpy stacks are used
# in_df: input dataframe
# batch_size: size of the batch of images
    ...
    while True:
        for (_, c_df) in np.random.permutation(all_groups):
            img_data = imread(os.path.join(
                map_img_dir, c_df['file_name'].values[0]))
            out_img += [img_data]
...
early = EarlyStopping(monitor="val_dice_coef",
                      mode="max",
                      patience=50)  # Be careful! Less patience leads to poor prediction/result, but more patience needs more time and has some system requirements
